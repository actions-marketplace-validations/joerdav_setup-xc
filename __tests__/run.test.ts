...
describe('Testing all functions in run file.', () => {
    test('run() must download specified xc version and set output', async () => {
        ...
        expect(core.setOutput).toHaveBeenCalledWith('xc-path', path.join('pathToCachedTool', 'xc'));
    });
    ...
    test('downloadXC() must download xc tarball, add it to github actions tool cache and return the path to extracted dir', async () => {
        ...
        expect(toolCache.find).toHaveBeenCalledWith('xc', 'v0.4.0');
        expect(toolCache.downloadTool).toHaveBeenCalled();
        expect(toolCache.cacheDir).toHaveBeenCalled();
        expect(os.type).toHaveBeenCalled();
        expect(fs.chmodSync).toHaveBeenCalledWith(path.join('pathToCachedTool', 'xc'), '777');
    });
    ...
    test('downloadXC() must download xc zip archive, add it to github actions tool cache and return the path to extracted dir', async () => {
        ...
        expect(toolCache.cacheDir).toHaveBeenCalled();
        expect(os.type).toHaveBeenCalled();
        expect(fs.chmodSync).toHaveBeenCalledWith(path.join('pathToCachedTool', 'xc.exe'), '777');
    });
